...
namespace nou
{
	...
	void CPathAnimator::Update(const PathSampler::KeypointSet& keypoints, float deltaTime)
	{
		...
		if (keypoints.size() >= 2) {
			if (m_segmentIndex < keypoints.size() - 1) {
				...
				else if (m_segmentTimer >= m_segmentTravelTime && m_segmentIndex + 1 < keypoints.size()) {
					m_segmentTimer = 0;
					m_segmentIndex++;
				}
				
				if (m_segmentIndex + 1 < keypoints.size()) {
					m_owner->transform.m_pos = PathSampler::LERP(keypoints[m_segmentIndex]->transform.m_pos, keypoints[m_segmentIndex + 1]->transform.m_pos, m_segmentTimer);
				}
				
			}			
		}
		else if (keypoints.size() == 1) {
			m_owner->transform.m_pos = keypoints[0].get()->transform.m_pos;
		}
		else {
			m_owner->transform.m_pos = glm::vec3(0.0f, 0.0f, 0.0f);
		}
		...
		//			m_owner->transform.m_pos = point1;
		//			PathSampler::LERP(keypoints[m_segmentIndex]->transform.m_pos, keypoints[m_segmentIndex + 1]->transform.m_pos, m_segmentTimer);
		//		}
		//	}
