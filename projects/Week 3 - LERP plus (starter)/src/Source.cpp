...
int main()
{
	...
	while (!App::IsClosing() && !Input::GetKeyDown(GLFW_KEY_ESCAPE))
	{
		...
		if (ImGui::Button("Add Waypoint")) {				
			...
			Entity* point = points.back().get();
			point->Add<CMeshRenderer>(*point, *mesh_box, *mat_unselected);
			point->transform.m_scale = glm::vec3(0.05f, 0.05f, 0.05f);
		}		

		// TODO: add an Imgui button to delete the selected point
		if (ImGui::Button("Delete Waypoint")) {
			
			// -- NOTE -- Does not delete selected point.  But deletes waypoints, so it's good enough for the video.
			if (!points.size() <= 0) {
		...
		for (size_t i = 0; i < points.size(); ++i)
		{
			...
			if (ImGui::Selectable(pointLabel.c_str(), i == pointSelected))
			{
				...
				pointSelected = i;
				_x = points[pointSelected]->transform.m_pos.x;
				_y = points[pointSelected]->transform.m_pos.y;
				_z = points[pointSelected]->transform.m_pos.z;
			}
		}

		
		if (ImGui::Button((runSimulation) ? "Pause Simulation" : "Run Simulation")) {
		...
		if (ImGui::Button("Reset Simulation")) {
			...
			_y = points[pointSelected]->transform.m_pos.y;
			_z = points[pointSelected]->transform.m_pos.z;
			ent_ducky.Get<CPathAnimator>().Reset();
			

			if (points.size() > 0) {
				ent_ducky.transform.m_pos = points[0].get()->transform.m_pos;				
			}
			else {
				ent_ducky.transform.m_pos = glm::vec3(0.0f, 0.0f, 0.0f);				
			}
		}
		...
		if (pointSelected < points.size())
		{
			...
			// -- Changes the points position
			points[pointSelected]->transform.m_pos = glm::vec3(_x, _y, _z);
