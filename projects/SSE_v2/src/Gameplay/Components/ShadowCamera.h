...
public:
	...
	// Orthographic Variables Added in Week 9
	bool Orthographic;
	inline void SetOrthographic(bool state) { Orthographic = state; Refresh(); }
		
	glm::vec4 Size; // x = L, y = R, z = B, w = T
	inline void SetSize(glm::vec4 newSize) { Size = newSize; Refresh(); }
	
	// Perspective Variables -- Added in Week 9
	float AspectRatio;
	inline void SetAspectRatio(float newAspect) { AspectRatio = newAspect; Refresh(); }
	
	float FieldOfView;
	inline void SetFieldOfView(float newFOV) { FieldOfView = newFOV; Refresh(); }
	
	// Shared Variables -- Added in Week 9
	glm::vec2 Planes; // x = near plane, y = far plane
	inline void SetPlanes(glm::vec2 newPlanes) { Planes = newPlanes; Refresh(); }
