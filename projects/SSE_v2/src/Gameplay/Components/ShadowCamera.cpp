...
// Added in Week 9
void ShadowCamera::Refresh()
{
	if (Orthographic) {
		SetProjection(glm::ortho(Size.x, Size.y, Size.z, Size.a, Planes.x, Planes.y));
	}
	else {
		SetProjection(glm::perspective(glm::radians(FieldOfView), AspectRatio, Planes.x, Planes.y));
	}
}
...
void ShadowCamera::RenderImGui()
{
	...
	bool b_Ortho = Orthographic;
	ImGui::Checkbox("Orthographic?", &b_Ortho);
	if (Orthographic != b_Ortho) {		
		SetOrthographic(b_Ortho);			
	}
	...
	{		
		// Size
		glm::vec4 v_Size = Size;
		ImGui::InputFloat4("Size", &v_Size.x, 1);
		if (Size != v_Size) {			
			SetSize(v_Size);
		}
	}
	...
	{
		// FoV
		float f_FOV = FieldOfView;
		ImGui::InputFloat("Field of View", &f_FOV, 1);
		if (FieldOfView != f_FOV) {
			SetFieldOfView(f_FOV);
		}

		// Aspect
		float f_Aspect = AspectRatio;
		ImGui::InputFloat("Aspect Ratio", &f_Aspect, 1);
		if (AspectRatio != f_Aspect) {
			SetAspectRatio(f_Aspect);
		}
	}

	// Near and Far Plane
	glm::vec2 v_Planes = Planes;
	ImGui::InputFloat2("Planes", &v_Planes.x, 1);
	if (Planes != v_Planes) {
		SetPlanes(v_Planes);
	}
